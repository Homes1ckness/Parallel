...
public:
    ...
    // 从现有的模型文件中加载模型
    void load(string load_path);    // 对一个给定的口令进行切分
...
public:
    ...
    // 优先队列的初始化
    void init();    // 对优先队列的一个PT，生成所有guesses
    void Generate(PT pt);
    
    // MPI 并行版本的 Generate 方法
    void GenerateMPI(PT pt);
      // MPI 辅助方法：收集所有进程的猜测结果
    void mpi_gather_guesses(vector<string>& local_guesses, int rank, int size);
